...
for(i in 1:length(divs.ns.l)){
  ...
  divs.genres.l[[div.content]] <- word.v
...
barplot(hapax.percentage, main = "Hapax analysis in Measure for Measure (5a), 
        Henry VIII (6b), and Antony and Cleopatra (4c)",
        ylab = "lexical richness",
        xlab = "Number of markings per play",
        beside=T, col="grey", names.arg = c(v1, v2, v3))
