...
man.world.good <- cbind(man, world, good)
...
man.world.good[which(is.na(man.world.good))] <- 0
love.death[which(is.na(love.death))] <- 0
#specifies column names for x axis in barplot
colnames(man.world.good) <- c("man", "world", "good")
...
library(zoo)
z <- merge(zoo(man.world.good), zoo(love.death))
plot(z, main="Term Frequencies", xlab = "Each marking instance", 
     type = "h", 
     plot.type = "multiple", 
     col = 1, lwd = 4)
