...
hmMarked <- glue("358b-words.txt", sep = " ")
...
words_sentiment %>% ggplot(aes(x = index, y = net_sentiment, fill = file)) + 
  ...
  facet_wrap(~ file) + 
  scale_x_continuous("Location in Milton's works") + 
...
bing_word_counts %>%
  ...
  labs(y = "Marked Words in Milton's works",
       x = NULL) +
  coord_flip()
...
words %>%
  ...
  comparison.cloud(max.words = 10000, scale = c(2,.5), 
                   random.order = FALSE,
                   colors = c("red", "blue"))
