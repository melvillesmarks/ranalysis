...
divs.ns.l <- getNodeSet(doc, "/body//*[@attribution='HM'][@mode='history']
...
for(i in 1:length(divs.ns.l)){
  ...
  divs.history.l[[div.content]] <- word.v
...
history.hapax.v <- sapply(div.raws.l, function(x) sum(x == 1))
history.lengths.m <- do.call(rbind, lapply(div.raws.l,sum))
hapax.percentage <- history.hapax.v / history.lengths.m
