...
library(XML)
doc <- xmlTreeParse("278-markings-only.xml", useInternalNodes=TRUE)
divs.ns.l <- getNodeSet(doc, "/body//div")
divs.ns.l
div.freqs.l <- list()
div.raws.l <- list()
...
write.table(sorted.div.freqs.v, "278.freqs.v.sorted.txt")
...
plot(mean.word.use.m, type = "h", main = "Mean word usage patterns in Melville's markings in Homer's 'Odyssey'",
...
plot(scale(mean.word.use.m), type = "h", main = "Scaled mean word usage patterns in Melville's markings in Homer's 'Odyssey'",
     ylab = "mean word use", xlab = "nodes (each marking)") 
...
plot(ttr.m, type = "h", main = "Type-token ratios in Melville's markings in Homer's 'Odyssey'",
